...
| **Speed Dial** | Sets fade/duration timings on attached functions live; tap-tempo. | AbsoluteValue range; Tap/Mult/Div; per-function multiplier (None…Sixteen, One=6) applied to FadeIn/FadeOut/Duration; Time value. |
...
**Beat generator** (on the I/O map): a global tempo source — `Internal`, `MIDI`, or `Audio` — at a set `BPM`. Functions in Beats tempo mode lock to it, so chases/matrix effects run on the beat.
